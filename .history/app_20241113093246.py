...
def register_user():
    if request.method == 'POST':
        ...
        connection = get_db_connection()
        with connection.cursor() as cursor:
            sql = "INSERT INTO Users (Name, ContactInfo, PreferredTransportMode, Password) VALUES (%s, %s, %s)"
            cursor.execute(sql, (name, contact_info, preferred_transport))
...
def process_booking():
    ...
    with connection.cursor() as cursor:
        ...
        if transport_mode['CurrentOccupancy'] < transport_mode['Capacity']:
            ...
            cursor.execute("""
            INSERT INTO Journeys (StartLocation, EndLocation, StartTime, JourneyCost, TransportModeID)
            VALUES ('Unknown Start', 'Unknown End', NOW(), %s, %s)
            """, (journey_cost, transport_mode_id))
            connection.commit()
