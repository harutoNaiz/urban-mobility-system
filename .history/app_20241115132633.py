...
def suggest_routes():
    ...
    connection = get_db_connection()
    with connection.cursor() as cursor:
        # Modified query to include ModeType from TransportModes
        cursor.execute("""
            SELECT 
                r.TransportModeID,
                tm.ModeType as VehicleName,
                r.Distance,
                ROUND((r.Distance * 2.5), 2) as Cost  -- Assuming cost calculation
            FROM Routes r
            JOIN TransportModes tm ON r.TransportModeID = tm.TransportModeID
            WHERE r.RouteID = %s 
            AND (r.Distance * 2.5) <= %s
        """, (route_id, budget))
