    ...
    with connection.cursor() as cursor:
        ...
        if transport_mode['CurrentOccupancy'] < transport_mode['Capacity']:
            ...
            if route:
                ...
                cursor.execute("""
                INSERT INTO Journeys (RouteID, StartTime, JourneyCost, TransportModeID)
                VALUES (%s, NOW(), %s, %s)
                """, (route_id, journey_cost, transport_mode_id))
                connection.commit()
                
                # Update occupancy
                cursor.execute("UPDATE TransportModes S-----ET CurrentOccupancy = CurrentOccupancy + 1 WHERE TransportModeID = %s", (transport_mode_id,))
                connection.commit()
