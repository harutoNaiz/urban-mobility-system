...
app = Flask(__name__)
app.secret_key = 'your_secret_key'  # Set a unique secret key for the session
...
def select_route():
    ...
    if request.method == 'POST':
        ...
        if traffic_warning:
            ...
            return redirect(url_for('suggest_routes', route_id=route_id))
    
    # Fetch available routes for the user
    connection = get_db_connection()
    with connection.cursor() as cursor:
...
def suggest_routes():
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute(sql, (route_id, route_id, budget, user_id))  # Pass the user_id as a parameter
